...
import net.floodlightcontroller.map.MapRule;
import net.floodlightcontroller.map.MappingTableManager;
...
IOFSwitchListener{
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		//IPv6Address  dst_eid =  pi.getMatch().get(MatchField.IPV6_DST);"ipv6_dst"  instead  of  destination   edge host  IPv6  Address
		IPv6Address  dst_eid = IPv6Address.of("fe80::231:32ff:fe33:6090");
		MappingTableManager   map = new MappingTableManager();
	    String  dst_rloc  =  map.getRules(dst_eid.toString());//According to  EID   query  destination  RLOC
		if(dst_rloc != null){
			log.info("SXT_DBG:********query  dst_rloc  success,will   doFlowMod****");
			doFlowMod(sw, pi, cntx,dst_eid,IPv6Address.of(dst_rloc));
		}else{
